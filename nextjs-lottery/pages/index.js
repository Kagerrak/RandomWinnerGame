  ...
  */
  ...
  const startGame = async () => {
    try {
      ...
      const randmomWinnerGameContract = new Contract(
      ...
      const tx = await randmomWinnerGameContract.startGame(
        maxPlayers,
        entryFee
      );
      await tx.wait(1);
  ...
  const joinGame = async () => {
    try {
      ...
      setLoading(true);
      const tx = randmomWinnerGameContract.joinGame({ value: entryFee });
      await tx.wait(1);
      setLoading(false);
    } catch (error) {
      console.error(error);
  ...
  const checkIfGameStarted = async () => {
    try {
      ...
      const _gameStarted = await randmomWinnerGameContract.gameStarted();
  ...
  const getOwner = async () => {
    try {
      ...
      const _owner = await randmomWinnerGameContract.owner();
  ...
  */
  ...
  return (
    <div className={styles.container}>
      ...
      <footer className={styles.footer}>Made with &#10084; by DevKage</footer>
    </div>
  );
}
